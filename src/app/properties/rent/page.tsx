...
export default function Search() {
  ...
  const [propertyStatus, _setPropertyStatus] = useState<string>("rent")
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("/api/properties/statusId=2list?statusProperty=ACCEPTED")
        if (!response.ok) {
  ...
  const filteredProperties = properties
    .filter((property) => {
      ...
      return textMatch  && bedroomsMatch && bathroomsMatch && priceMatch
    })
    .sort((a, b) => {
    ...
    <>
        ...
        <div className="relative w-full px-[10%] py-[3%]">
          <div className="max-w-4xl">
            <div className="text-sm text-slate-600 mb-2">
              ZYCK Property &gt; Properties for Rent
            </div>
      ...
      <div className="mt-5 flex flex-col md:flex-row">
        ...
        <div className="md:w-1/4 w-full p-3 space-y-4 rounded-lg">
          ...
          <Select value={propertyStatus} defaultValue={propertyStatus}>
            <SelectTrigger className="w-full p-2 border border-gray-300 rounded">
